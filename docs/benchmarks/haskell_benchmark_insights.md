...
2. **Fasta: `massiv` library not explicitly installed** *(fixed)*. `fasta/main.hs` imports `Data.Massiv.Array.Mutable`, `Data.Massiv.Array.Unsafe`, and `Data.Massiv.Array` (lines 6–8). `cabal install --lib massiv` has been added to `fasta.yml` and `fasta_test.yml` setup-commands to resolve this.
